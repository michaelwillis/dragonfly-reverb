...
typedef struct
{
  ...
  sr2_float_t left_calc [128], right_calc [128];
...
{
  // if(sizeof (increment_t) == 8) return (llrint ((x) * FP_ONE));
...
static int SR2_(prepare_data)(SR2_(SINC_FILTER) * filter,
			       SR2_(SRC_DATA) *data, long half_filter_chan_len)
{
  long len = 0;
  
  if(filter->b_real_end >= 0) return 0;
...
static inline sr2_float_t
SR2_(calc_output_single) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index)
{	sr2_float_t		fraction, left, right, icoeff ;
...
static long
SR2_(sinc_mono_vari_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data)
{	SR2_(SINC_FILTER) *filter ;
	sr2_float_t	input_index, src_ratio, count, float_increment, terminate, rem ;
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		if (samples_in_hand <= half_filter_chan_len)
		  {	if ((psrc->error = SR2_(prepare_data) (filter, data, half_filter_chan_len)) != 0)
				return psrc->error ;
		...
		data->data_out [filter->out_gen] = (sr2_float_t) ((float_increment / filter->index_inc) *
							    SR2_(calc_output_single) (filter, increment, start_filter_index)) ;
		filter->out_gen ++ ;
...
static inline void
SR2_(calc_output_stereo) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index, sr2_float_t scale, sr2_float_t * output)
{	sr2_float_t	fraction, left [2], right [2], icoeff ;
	increment_t	filter_index, max_filter_index ;
	int			data_index, coeff_count, indx ;
...
static long
SR2_(sinc_stereo_vari_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data)
{	SR2_(SINC_FILTER) *filter ;
	sr2_float_t	input_index, src_ratio, count, float_increment, terminate, rem ;
	increment_t	increment, start_filter_index ;
	int			half_filter_chan_len, samples_in_hand ;
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		SR2_(calc_output_stereo) (filter, increment, start_filter_index, float_increment / filter->index_inc, data->data_out + filter->out_gen) ;
		filter->out_gen += 2 ;
...
static inline void
SR2_(calc_output_quad) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index, sr2_float_t scale, sr2_float_t * output)
{	sr2_float_t	fraction, left [4], right [4], icoeff ;
	increment_t	filter_index, max_filter_index ;
	int			data_index, coeff_count, indx ;
...
static long
SR2_(sinc_quad_vari_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data)
{	SR2_(SINC_FILTER) *filter ;
	sr2_float_t	input_index, src_ratio, count, float_increment, terminate, rem ;
	increment_t	increment, start_filter_index ;
	int			half_filter_chan_len, samples_in_hand ;
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		SR2_(calc_output_quad) (filter, increment, start_filter_index, float_increment / filter->index_inc, data->data_out + filter->out_gen) ;
		filter->out_gen += 4 ;
...
static inline void
SR2_(calc_output_hex) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index, sr2_float_t scale, sr2_float_t * output)
{	sr2_float_t	fraction, left [6], right [6], icoeff ;
	increment_t	filter_index, max_filter_index ;
	int			data_index, coeff_count, indx ;
...
static long
SR2_(sinc_hex_vari_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data)
{	SR2_(SINC_FILTER) *filter ;
	sr2_float_t	input_index, src_ratio, count, float_increment, terminate, rem ;
	increment_t	increment, start_filter_index ;
	int			half_filter_chan_len, samples_in_hand ;
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		SR2_(calc_output_hex) (filter, increment, start_filter_index, float_increment / filter->index_inc, data->data_out + filter->out_gen) ;
		filter->out_gen += 6 ;
...
static inline void
SR2_(calc_output_multi) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index, int channels, sr2_float_t scale, sr2_float_t * output)
{	sr2_float_t		fraction, icoeff ;
	/* The following line is 1999 ISO Standard C. If your compiler complains, get a better compiler. */
	sr2_float_t		*left, *right ;
	increment_t	filter_index, max_filter_index ;
...
static long
SR2_(sinc_multichan_vari_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data)
{	SR2_(SINC_FILTER) *filter ;
	sr2_float_t	input_index, src_ratio, count, float_increment, terminate, rem ;
	increment_t	increment, start_filter_index ;
	int			half_filter_chan_len, samples_in_hand ;
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		SR2_(calc_output_multi) (filter, increment, start_filter_index, filter->channels, float_increment / filter->index_inc, data->data_out + filter->out_gen) ;
...
long SR2_(sinc_set_converter) (SR2_(SRC_PRIVATE) *psrc, long src_enum)
{
  ...
  if (psrc->channels > ARRAY_LEN (temp_filter.left_calc))
    ...
    {
      psrc->const_process = SR2_(sinc_mono_vari_process) ;
      psrc->vari_process = SR2_(sinc_mono_vari_process) ;
    }
  else if (psrc->channels == 2)
    {
      psrc->const_process = SR2_(sinc_stereo_vari_process) ;
      psrc->vari_process = SR2_(sinc_stereo_vari_process) ;
    }
  else if (psrc->channels == 4)
    {
      psrc->const_process = SR2_(sinc_quad_vari_process) ;
      psrc->vari_process = SR2_(sinc_quad_vari_process) ;
    }
  else if (psrc->channels == 6)
    {
      psrc->const_process = SR2_(sinc_hex_vari_process) ;
      psrc->vari_process = SR2_(sinc_hex_vari_process) ;
    }
  else
    {
      psrc->const_process = SR2_(sinc_multichan_vari_process) ;
      psrc->vari_process = SR2_(sinc_multichan_vari_process) ;
    }
  ...
  */
  ...
  if((filter = calloc (1, sizeof (SR2_(SINC_FILTER)) +
		       sizeof (filter->buffer [0]) *
		       (temp_filter.b_len + temp_filter.channels))) == NULL)
